...
void main() {
  ...
  float attenuation =
      1.0f / (myLight.c + myLight.l * dis + myLight.q * dis * dis);
  // ambient
  vec3 _ambient =
      myLight.m_ambient * vec3(texture(myMaterial.texture_diffuse1, outUV));
  ...
  vec3 _diffuse = myLight.m_diffuse * _diff *
                  vec3(texture(myMaterial.texture_diffuse1, outUV));
  ...
  float _spec =
      pow(max(dot(_viewDir, _reflectDir), 0.0f), myMaterial.m_shiness);

  vec3 _sepcular = myLight.m_specular * _spec *
                   vec3(texture(myMaterial.texture_specular1, outUV));

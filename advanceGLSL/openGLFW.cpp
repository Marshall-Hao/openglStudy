...
void rend()
{
  ...
  glBufferSubData(GL_UNIFORM_BUFFER, sizeof(glm::mat4), sizeof(glm::mat4),
                  glm::value_ptr(_camera.getMatrix()));
...
void initShader()
{
  _shader.initShader("shader/advVertexShader.glsl",
                     "shader/advFragmentShader.glsl");
  _shader_red.initShader("shader/redShaderv.glsl", "shader/redShaderf.glsl");
  _shader_green.initShader("shader/greenShaderv.glsl",
                           "shader/greenShaderf.glsl");
}
...
uint createUBO()
{
  // create shader storage buffer object, for share data between shader
  uint _ubo = 0;
  glGenBuffers(1, &_ubo);
  // bind buffer to GL_UNIFORM_BUFFER
...
void bindShaderData()
{
  // bind shader storage buffer object to binding point, this case is use 0 as
  // the binding point
  glBindBufferBase(GL_UNIFORM_BUFFER, 0, UBO);

  // get the index of uniform block in shader
  uint redIndex = glGetUniformBlockIndex(_shader_red.getProgram(), "MAT");
  // bind the index to binding point
...
int main()
{
  ...
  _camera.lookAt(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, -1.0f),

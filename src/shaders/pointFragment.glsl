...
struct Light
{
 ...
 // the attenuation of light 光线衰减参数 
 float m_constant;
 float m_linear;
 float m_quadratic;
};
...
void main()
{   
    float _attenuation =1.0f /(myLight.m_constant + myLight.m_linear * length(myLight.m_position - outFragPos) + myLight.m_quadratic * pow(length(myLight.m_position - outFragPos), 2));
    // ambient
    ...
    FragColor = texture(tex, outUV) * vec4(result,1.0) * _attenuation;
}